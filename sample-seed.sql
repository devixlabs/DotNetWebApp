...
INSERT INTO Producks (Name, Description, Price, CategoryId)
SELECT 
    'Wireless Mouse', 
...
WHERE EXISTS (SELECT 1 FROM Categories WHERE Name = 'Electronics')
  AND NOT EXISTS (SELECT 1 FROM Producks WHERE Name = 'Wireless Mouse');
...
WHERE EXISTS (SELECT 1 FROM Categories WHERE Name = 'Books')
  AND NOT EXISTS (SELECT 1 FROM Producks WHERE Name = 'Programming in C#');
...
WHERE EXISTS (SELECT 1 FROM Categories WHERE Name = 'Outdoor')
  AND NOT EXISTS (SELECT 1 FROM Producks WHERE Name = 'Camping Lantern');
