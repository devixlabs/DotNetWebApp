...
public class MultiAppRoutingTests
{
    ...
    #region ApplicationContextAccessor Tests

    [Theory]
    [InlineData("/admin/dashboard", "admin")]
    [InlineData("/admin/entity/acme/Product", "admin")]
    [InlineData("/reporting/dashboard", "reporting")]
    [InlineData("/metrics/entity/initech/User", "metrics")]
    public void ApplicationContextAccessor_ExtractsAppNameFromPath(string path, string expectedApp)
    {
        // Arrange
        ...
        var contextAccessor = new ApplicationContextAccessor(mockHttpContextAccessor.Object, _mockAppDictionary.Object);

        // Act
        var appName = contextAccessor.ApplicationName;

        // Assert
        Assert.Equal(expectedApp, appName);
    }

    [Theory]
    [InlineData("/css/app.css")]
    [InlineData("/js/app.js")]
    [InlineData("/lib/bootstrap/css/bootstrap.css")]
    [InlineData("/_framework/blazor.server.js")]
    [InlineData("/images/logo.png")]
    [InlineData("/api/some-endpoint")]
    public void ApplicationContextAccessor_ReturnsNullForStaticPaths(string staticPath)
    {
        // Arrange
        var httpContext = new DefaultHttpContext();
        httpContext.Request.Path = staticPath;
    ...
    [Fact]
    public void ApplicationContextAccessor_ReturnsNullForInvalidApp()
    {
        // Arrange
        var httpContext = new DefaultHttpContext();
        httpContext.Request.Path = "/invalidapp/dashboard";
    ...
    [Fact]
    public void ApplicationContextAccessor_CachesResultPerRequest()
    {
        // Arrange
        var httpContext = new DefaultHttpContext();
        ...
        // Act
        var appName1 = contextAccessor.ApplicationName;
        var appName2 = contextAccessor.ApplicationName;

        // Assert - both calls should return same value (cached)
        Assert.Equal(appName1, appName2);
        Assert.Equal("admin", appName1);

        // Verify GetApplication was called at least once (caching works)
        _mockAppDictionary.Verify(x => x.GetApplication("admin"), Times.AtLeastOnce);
    }
