...
public interface IFIFOAllocationEngine
{
    ...
    /// </summary>
    Task<InventoryAllocationResult> AllocateAsync(
        string orderNumber,
        string productCode,
...
/// Uses dtfifo table for inventory, webapp_allocate for allocations.
/// </summary>
public class FIFOAllocationEngine : IFIFOAllocationEngine
{
    private readonly SecondaryDbContext _context;
    private readonly IDapperQueryService _primaryDapper;   // WEBAPP database - for dtfifo
    private readonly IDapperQueryService _secondaryDapper; // WEBAPPMisc database - for webapp_allocate
        ...
        DateTime orderDate)
    {
        ...
        if (!lots.Any())
        {
            ...
            return InventoryAllocationResult.Failed("No available inventory after restrictions/shelf-life filters");
        }
        ...
        foreach (var lot in lots)
        {
            ...
            if (toAllocate > 0)
            {
                ...
                await InsertAllocationAsync(new InventoryAllocationRecord
                {
    ...
    private async Task InsertAllocationAsync(InventoryAllocationRecord record)
    {
        var allocation = new Webapp_allocate
        {
        ...
        _context.Set<Webapp_allocate>().Add(allocation);
