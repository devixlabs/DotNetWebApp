...
using DotNetWebApp.Services.PrePick.Validators;
...
/// <summary>
/// Implementation of PrePick service for order picking/auditing/assembly workflow.
/// Uses EF Core for writes (INSERT/UPDATE) and Dapper for complex reads.
/// Uses TWO Dapper services: Primary (GAI) for Deacom queries, Secondary (GAIMisc) for scheduler queries.
/// </summary>
public class PrePickService : IPrePickService
{
    private readonly SecondaryDbContext _context;
    private readonly IDapperQueryService _primaryDapper;   // GAI database - for dttord, dmprod queries
    private readonly IDapperQueryService _secondaryDapper; // GAIMisc database - for gai_scheduler queries
    private readonly AuditorAssignmentValidator _auditorValidator;
    private readonly TimestampValidator _timestampValidator;
    private readonly ILogger<PrePickService> _logger;

    public PrePickService(
    ...
    public async Task<PrePickOrder?> GetOrderAsync(string orderNumber, int warehouseId)
    {
        long ordNum = long.Parse(orderNumber);

        // Get scheduler data
        var scheduler = await _context.Set<Gai_scheduler>()
    ...
    public async Task<IEnumerable<PrePickOrder>> ListOrdersAsync(
        int warehouseId,
        ...
        DateTime? endDate = null)
    {
        ...
        // Query from gai_scheduler for PrePick orders
        ...
        var orders = await _secondaryDapper.QueryAsync<PrePickOrder>(sql, new
        {
    ...
    {
        // Query from gai_dmstech for users with allocator flag
        const string sql = @"
    ...
    {
        // Query from gai_dmstech for users with team leader flag
        const string sql = @"
            SELECT DISTINCT gs_chr1 AS Username
            FROM gai_dmstech
            WHERE gs_chr1 IS NOT NULL AND gs_chr1 <> ''
    ...
    private async Task<Gai_scheduler> GetOrCreateSchedulerAsync(long ordNum, int warehouseId)
    {
        var scheduler = await _context.Set<Gai_scheduler>()
            .FirstOrDefaultAsync(s => s.gs_ordnum == ordNum && s.gs_id == warehouseId);
        ...
        if (scheduler == null)
        {
            ...
            _context.Set<Gai_scheduler>().Add(scheduler);
        }

        return scheduler;
    }

    private static PrePickOrder MapSchedulerToOrder(Gai_scheduler scheduler)
    {
        return new PrePickOrder
        {
