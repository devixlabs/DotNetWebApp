...
/// <summary>
/// Service interface for PrePick workflow operations.
/// Manages picker, auditor, and assembler assignments for orders.
/// </summary>
public interface IPrePickService
{
    /// <summary>
    /// Get a single PrePick order by order number.
    /// </summary>
    Task<PrePickOrder?> GetOrderAsync(string orderNumber, int warehouseId);

    /// <summary>
    /// List PrePick orders for a warehouse within a date range.
    /// </summary>
    Task<IEnumerable<PrePickOrder>> ListOrdersAsync(
        int warehouseId,
