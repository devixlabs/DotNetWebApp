...
/// <summary>
/// Implementation of allocation service for inventory allocation operations.
/// Uses EF Core for writes, Dapper for complex reads.
/// Uses TWO Dapper services: Primary (GAI) for Deacom queries, Secondary (GAIMisc) for allocation queries.
/// </summary>
public class AllocationService : IAllocationService
{
    private readonly SecondaryDbContext _context;
    private readonly IDapperQueryService _primaryDapper;   // GAI database - for dttord, dmprod, dtfifo
    private readonly IDapperQueryService _secondaryDapper; // GAIMisc database - for gai_allocate, gai_lock
    ...
    public async Task<IEnumerable<AllocationOrderSummary>> GetOrdersForAllocationAsync(
        int warehouseId,
        DateTime? shipDate = null)
    {
        ...
        const string sql = @"
            SELECT
                ...
                (SELECT COUNT(DISTINCT all_codenum) FROM GAIMisc.dbo.gai_allocate
                ...
                (SELECT ISNULL(SUM(all_qty), 0) FROM GAIMisc.dbo.gai_allocate
                ...
                ISNULL((SELECT TOP 1 gl_username FROM GAIMisc.dbo.gai_lock
        ...
        var orders = await _primaryDapper.QueryAsync<AllocationOrderSummary>(sql, new
        {
        ...
        {
            order.Status = AllocationStatusColorService.GetStatus(order.TotalOrdered, order.TotalAllocated);
        }

        return orders;
    }

    public async Task<AllocationOrderSummary?> GetOrderDetailAsync(string orderNumber, int warehouseId)
    {
        ...
        var order = await _primaryDapper.QuerySingleAsync<AllocationOrderSummary?>(sql, new
        {
            OrderNumber = ordNum,
            WarehouseId = warehouseId
        });
    ...
    public async Task<IEnumerable<AllocationLineItem>> GetOrderLineItemsAsync(string orderNumber, int warehouseId)
    {
        long ordNum = long.Parse(orderNumber);
        ...
        {
            line.Status = AllocationStatusColorService.GetStatus(line.OrderedQuantity, line.AllocatedQuantity);
    ...
    public async Task<AllocationResult> AllocateOrderAsync(
        string orderNumber,
        string username,
        int warehouseId)
    {
        ...
        {
            return AllocationResult.Failed(eligibility.Reason);
        }
        ...
        {
            return AllocationResult.Failed($"Order is locked by {lockResult.LockedBy}");
        }
        ...
        try
        {
            ...
            if (totalShort > 0)
            {
                return AllocationResult.Partial(totalAllocated, totalShort);
            }

            return AllocationResult.Succeeded(totalAllocated);
        }
        finally
        {
            // Always release lock
    ...
    public async Task<AllocationResult> AllocateLineItemAsync(
        string orderNumber,
        string productCode,
        decimal quantity,
        string username,
    ...
    public async Task<bool> DeallocateOrderAsync(string orderNumber, string username, int warehouseId)
    {
        ...
        try
        {
            ...
            var allocations = await _context.Set<Gai_allocate>()
            ...
            if (allocations.Any())
            {
                _context.Set<Gai_allocate>().RemoveRange(allocations);
    ...
    public async Task<AllocationStatus> GetOrderStatusAsync(string orderNumber, int warehouseId)
    {
        var order = await GetOrderDetailAsync(orderNumber, warehouseId);
    ...
    private async Task<IEnumerable<AllocationRecord>> GetLineAllocationsAsync(
        string orderNumber,
        string productCode,
        int warehouseId)
    {
        ...
        return await _secondaryDapper.QueryAsync<AllocationRecord>(sql, new
        {
            OrderNumber = ordNum,
    ...
    private async Task<bool> CheckPermissionAsync(string username, int permissionId)
    {
        ...
        const string sql = @"
            ...
            FROM dtd2
            JOIN gaisystem.dbo.dxuser ON us_id = d2_recid
