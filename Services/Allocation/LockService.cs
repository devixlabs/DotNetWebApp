...
/// Uses gai_lock table with gl_id = 1 for order locks.
...
public class LockService : ILockService
{
    ...
    {
        var userLocks = await _context.Set<Gai_lock>()
        ...
        if (userLocks.Any())
        {
            _context.Set<Gai_lock>().RemoveRange(userLocks);
    ...
    public async Task<OrderLock?> GetLockAsync(string orderNumber, int warehouseId)
    {
        ...
        var lockEntity = await _context.Set<Gai_lock>()
    ...
    public async Task<int> ReleaseExpiredLocksAsync()
    {
        ...
        var expiredLocks = await _context.Set<Gai_lock>()
            .Where(l => l.gl_id == ORDER_LOCK_TYPE &&
        ...
        if (expiredLocks.Any())
        {
            _context.Set<Gai_lock>().RemoveRange(expiredLocks);
            await _context.SaveChangesAsync();
    ...
    private async Task CreateLockAsync(long orderNumber, string username, int warehouseId)
    {
        ...
        _context.Set<Gai_lock>().Add(lockEntity);
        await _context.SaveChangesAsync();
    }
    ...
    private async Task ReleaseLockInternalAsync(long orderNumber, int warehouseId)
    {
        ...
        if (lockEntity != null)
        {
            _context.Set<Gai_lock>().Remove(lockEntity);
            await _context.SaveChangesAsync();
        }
    }
}
