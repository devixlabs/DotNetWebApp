...
/// <summary>
/// Provides color coding for DMS orders based on type and status
/// Used by Blazor UI for visual differentiation
/// </summary>
public class DMSColorCodingService
{
    /// <summary>
    ...
    {
        var (_, color) = DMSOrderTypeClassifier.GetTypeDisplay(type);
        return color;
    }
    ...
    /// </summary>
    /// <param name="order">DMS order</param>
    /// <returns>CSS style string for the row</returns>
    public static string GetRowStyle(DMSOrder order)
    {
    ...
    public static string GetOrderTypeBadgeStyle(OrderType type)
    {
        return type switch
        {
            ...
            OrderType.ICT_NL or OrderType.ICT_CPFG => "Warning",
            OrderType.Deleted_SO or OrderType.Deleted_PO or
            OrderType.Deleted_ICT_NL or OrderType.Deleted_ICT_CPFG => "Light",
            _ => "Light"
        };
    }
